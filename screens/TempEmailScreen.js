...
// Import API functions from the service file
...
export default function TempEmailScreen({ navigation }) {
  ...
  const [expiresAt, setExpiresAt] = useState(null) // Store as timestamp (number)
  const [messages, setMessages] = useState([])
  const [isGenerating, setIsGenerating] = useState(false) // For generate button loader
  const [isLoadingMessages, setIsLoadingMessages] = useState(false) // For initial message load indicator
  const [refreshing, setRefreshing] = useState(false) // For pull-to-refresh indicator
  const [error, setError] = useState(null) // To display errors to the user

  // --- Refs ---
  const pollingInterval = useRef(null) // To store the interval ID for cleanup
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (pollingInterval.current) {
        clearInterval(pollingInterval.current) // Clear the polling interval
      }
    }
  ...
  // Function to fetch messages, update state, and handle errors
  const fetchAndUpdateMessages = useCallback(async (currentToken, indicateLoading = false) => {
    ...
    try {
      ...
      console.error("[fetchAndUpdateMessages] Error:", err) // Log error source
      if (isMounted.current) {
  ...
  const startPolling = useCallback(
    (currentToken) => {
      ...
      if (pollingInterval.current) clearInterval(pollingInterval.current)
      fetchAndUpdateMessages(currentToken, true) // Immediate fetch with loader
      pollingInterval.current = setInterval(() => {
        fetchAndUpdateMessages(currentToken, false) // Background fetch
      }, 15000)
    },
  ...
  // Handler for the "Generate Temp Email" button press
  const handleGenerateEmail = useCallback(async () => {
    ...
    try {
      ...
      console.error("[handleGenerateEmail] Error:", err) // Log error source
  ...
  const onRefresh = useCallback(() => {
    ...
    setRefreshing(true)
    fetchAndUpdateMessages(token, false) // Fetch without main loader
  }, [token, fetchAndUpdateMessages])

  // Handler for copying the email address to clipboard
  ...
  // Formats the expiration timestamp into a readable time string
